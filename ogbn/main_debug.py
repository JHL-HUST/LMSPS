...
def main(args):
    ...
    dict0,dict1 = average_diameter(g,'P',5)
    # self_value_dict = propagate_self_value_gpu_parallel(g,'P',4,device='cuda:0')
    # if args.label_feats:
...
def parse_args(args=None):
    ...
    ## For pre-processing
    parser.add_argument("--emb_path", type=str, default='../data/complex_nars/ckpts/ComplEx_mag_0/')
    ...
    parser.add_argument("--no_ms", action='store_true', default=False)
...
if __name__ == '__main__':
    ...
    # val_res = np.sort(results[:,0])[-2:]
    # test_res = np.sort(results[:,1])[-2:]
    ...
    # # for item in results:
    # print(f'val: {mima[0]}', f'test: {mima[1]}')
    # print(f'val_mean: {np.mean(mima[0]):.2f}', f'val_std: {np.std(mima[0]):.2f}')
    # print(f'test_mean: {np.mean(mima[1]):.2f}', f'test_std: {np.std(mima[1]):.2f}')
