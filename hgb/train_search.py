...
def main(args):
    ...
    if args.dataset != 'Freebase':
        ...
        if args.neighbor_attention:
            ...
            # try:
            #     g = torch.load(f'./cache/{args.dataset}_g_prop_feat_dgl_hop{args.num_hops}.pt')
            # except:
        ...
        if args.two_layer:
            meta_adjs = hg_propagate_sparse_pyg(adjs, node_types, args.num_hops, max_length, extra_metapath, prop_feats=True, echo=True, prop_device='cpu')  #prop_device
            for k in meta_adjs.keys(): assert len(k) > 1, k
    ...
    for stage in [0]:
        ...
        if args.neighbor_attention:
            ...
            model = SeHGNN_DY(args.embed_size, args.hidden, num_classes, feats.keys(), label_feats.keys(), tgt_type,
                args.dropout, args.input_drop, args.att_drop, args.label_drop,
                args.n_layers_1, args.n_layers_2, args.act, args.residual, bns=args.bns, data_size=data_size, num_heads=args.nh, no_path=args.no_path,
                no_label=args.no_label, remove_transformer=args.remove_transformer, independent_attn=args.independent_attn)
        else:
            model = SeHGNN(args.embed_size, args.hidden, num_classes, feats.keys(), label_feats.keys(), tgt_type,
                args.dropout, args.input_drop, args.att_drop, args.label_drop,
                args.n_layers_1, args.n_layers_2, args.act, device, args.residual, args.identity, bns=args.bns, data_size=data_size, num_heads=args.nh, num_sampled=args.ns,
        ...
        for epoch in tqdm(range(args.stage)):
            ...
            if args.two_layer:
                ...
                loss_w, loss_a, acc_w, acc_a = train_search(model, feats, label_feats, labels_cuda, loss_fcn, optimizer_w, optimizer_a, train_loader, val_loader, epoch_sampled, evaluator, scalar=scalar)
            torch.cuda.synchronize()
            end = time.time()
            ...
            # paths_100, _ = model.sample(list(feats.keys()), list(label_feats.keys()), args.ratio, 100)
            # print(f"paths_top100: {paths_100}\n")
                ...
                if args.two_layer:
                    ...
                    for batch, batch_feats, batch_labels_feats, batch_mask in eval_loader:
                        ...
                        if with_mask:
                            ...
                            batch_mask = None
                        raw_preds.append(model(batch, batch_feats, idx, batch_labels_feats, batch_mask).cpu()) #id_paths
                ...
                if args.two_layer:
                    ...
                    for batch, batch_feats, batch_labels_feats, batch_mask in full_loader:
                        ...
                        if with_mask:
                            ...
                            batch_mask = None
                        raw_preds.append(model(batch, batch_feats, idx, batch_labels_feats, batch_mask).cpu())
                raw_preds = torch.cat(raw_preds, dim=0)
...
def parse_args(args=None):
    ...
    parser.add_argument("--embed-size", type=int, default=512,  # 256
