    ...
    for stage in [0]:
        ...
        if args.neighbor_attention:
            ...
            model = SeHGNN(args.embed_size, args.hidden, num_classes, feats.keys(), label_feats.keys(), tgt_type,
                args.dropout, args.input_drop, args.att_drop, args.label_drop,
                args.n_layers_1, args.n_layers_2, args.act, args.residual, bns=args.bns, data_size=data_size, num_heads=args.nh, num_sampled=args.ns,
        ...
        for epoch in tqdm(range(args.stage)):
            ...
            meta_sampled, label_sampled = model.epoch_sample()
            if args.two_layer:
                loss, acc = train_2l(model, feats, label_feats, labels_cuda, loss_fcn, optimizer, train_loader, evaluator, tgt_type, scalar=scalar)
            else:
                loss_w, loss_a, acc_w, acc_a = train_search_two(model, feats, label_feats, labels_cuda, loss_fcn, optimizer_w, optimizer_a, train_loader, val_loader, meta_sampled, label_sampled, evaluator, scalar=scalar)
            torch.cuda.synchronize()
            end = time.time()

            # masked
            paths = model.sample(keys, list(label_feats.keys()), args.ratio, args.label_ratio)
            print("paths {}\n".format(paths))
                ...
                if args.two_layer:
                    ...
                    for batch, batch_feats, batch_labels_feats, batch_mask in eval_loader:
                        ...
                        if with_mask:
                            ...
                            batch_mask = None
                        raw_preds.append(model(batch, batch_feats, meta_sampled, label_sampled, batch_labels_feats, batch_mask).cpu())
...
def parse_args(args=None):
    ...
    parser.add_argument("--label_ratio", type=float, default=0)
