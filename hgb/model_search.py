...
class SeHGNN(nn.Module):
    def __init__(self, nfeat, hidden, nclass, feat_keys, label_feat_keys, tgt_type,
                 ...
                 residual=False, identity=False, bns=False, data_size=None, drop_metapath=0., 
                 num_heads=1, num_sampled = 1, remove_transformer=True, independent_attn=False):
        ...
        if self.remove_transformer:
            ...
            # self.layer_final = nn.Linear(hidden, hidden)   #注意要初始化参数
            self.layer_final = nn.Identity()
    ...
    def forward(self, batch, feature_dict, epoch_sampled, label_dict={}, mask=None):
        ...
        if isinstance(feature_dict[self.tgt_type], torch.Tensor):
            mapped_feats = {k: self.input_drop(x @ self.embeding[k]) for k, x in feature_dict.items() if k in meta_path_sampled or k==self.tgt_type}
        ...
        features = [ws[i] * features[i] for i in range(len(epoch_sampled))]
        ...
        features = features.sum(1)#features.mean(1)


        out = self.layer_final(features)
    ...
    def sample(self, keys, label_keys, lam, topn, all_path=False): #, topn
        '''
